#' Parse Tracking Football Data for NCAA Players
#'
...
get_all_ncaa_player_details <- function(api_key,
  ...
  count_resp <- GET(
    ...
    query = list(
      ...
      playerType = "college"     # Filter for player types
    ),
  ...
  for (page in seq_len(total_pages)) { # Loop through X number of pages
    ...
    player_page <- fromJSON(content(resp, as = "text", encoding = "UTF-8")) # Parse JSON; returns a huge fucking list or data.frame
